...
describe("API service", () => {
    ...
    it("gets time estimates for a stop", async () => {
        ...
        expect(data.predictionResponse["bustime-response"].prd[0].stpid).toEqual("456");
    ...
    it("gets the full pattern for a route", async () => {
        ...
        expect(data.patternResponse["bustime-response"].ptr[0].rtdir).toEqual("Eastbound");
        expect(data.patternResponse["bustime-response"].ptr[0].pt[0].stpnm).toEqual("Division & Austin Terminal");
        expect(data.patternResponse["bustime-response"].ptr[1].rtdir).toEqual("Westbound");
        expect(mockedAxios.get).toHaveBeenCalledWith(
    ...
    it("gets all stops for a single bus route", async () => {
        ...
        expect(data.stopsResponse["bustime-response"].stops[0].stpid).toEqual("4727");
        expect(mockedAxios.get).toHaveBeenCalledWith(
    ...
    it("gets available directions for a bus route", async () => {
        ...
        expect(data.directionsResponse["bustime-response"].directions.length).toEqual(2);
